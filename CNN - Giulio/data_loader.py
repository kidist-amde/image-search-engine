...
def load_dataset(data_path = './dataset', norm=True):
    ...
    def load_data(mypath):
        ...
        for i, cla in enumerate(mypath):
            filelist = glob.glob(os.path.join(cla, '*.jpg')) # path of all images in folder cla
            tmp_data = np.empty((len(filelist), 96, 96, 3), dtype='uint8') # temp array where we store images
...
def augmentation(dataset, labels):
    ...
    aug_data = np.zeros((0, 96, 96, 3), dtype='uint8')
    temp_data = np.empty((9, 96, 96, 3), dtype='uint8')
